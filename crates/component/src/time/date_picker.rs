...
impl DatePickerState {
    ...
    /// Opens or closes the popup with focus restoration; internal counterpart
    /// of the public [`Self::set_open`].
    ...
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        BaseDatePicker::new(self.id, &state.focus_handle)
            ...
            .on_open_change(move |open, window, cx| {
                picker_state.update(cx, |state, cx| state.set_open_internal(open, window, cx));
            })
            .key_context(CONTEXT)
