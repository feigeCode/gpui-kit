...
use gpui_base::{DialogStateChanged, TextSelection, TextSelectionScopeId};
...
impl WindowState {
        ...
        F: Fn(Dialog, &mut Window, &mut App) -> Dialog + 'static,
    {
        ...
        Self::emit_dialog_state_changed(self.active_dialogs.len(), window, cx);
        cx.notify();
    }
    ...
    /// Reports dialog count changes on the Base Root so applications can
    /// subscribe to modality changes without knowing this plugin.
    fn emit_dialog_state_changed(
        active_count: usize,
        window: &mut Window,
        cx: &mut Context<'_, WindowState>,
    ) {
        if let Some(root) = window.root::<gpui_base::Root>().flatten() {
            root.update(cx, |_, cx| {
                cx.emit(DialogStateChanged { active_count });
            });
        }
    }
